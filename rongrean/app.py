...
def update_map(click_data):
    fig = px.choropleth_mapbox(
        ...
        featureidkey='properties.name',
        color='value',
        color_continuous_scale="Plasma",  # เปลี่ยนสีเป็น Plasma
        mapbox_style="carto-positron",
        center={"lat": 13.736717, "lon": 100.523186},
        zoom=5
    )
    ...
    if click_data:
        ...
        fig.add_trace(
            px.scatter_mapbox(
                data_frame=df[df['province'] == selected_province],
                lat=[13.736717],
                lon=[100.523186],
                text=[selected_province],
                customdata=[df[df['province'] == selected_province]['value'].values[0]],
            ).data[0]
        )
...
def display_province_data(click_data):
    if click_data:
        ...
        return html.Div([
            ...
            html.P(f"ค่า: {province_value}")
        ])
    return "คลิกที่จังหวัดเพื่อดูข้อมูลเพิ่มเติม."
